...
test('dependencies fail closed and recover without losing manual retries', async ({ page }) => {
  ...
  await compose('stop', 'postgres-message')
  await page.getByPlaceholder('Digite uma mensagem...').fill(`Database retry ${suffix}`)
  await page.getByRole('button', { name: 'Enviar', exact: true }).click()
  const databaseMessage = page.locator('.message.mine')
    .filter({ hasText: `Database retry ${suffix}` })
  await expect(databaseMessage).toContainText('failed', { timeout: 45_000 })
  await compose('start', 'postgres-message')
  await waitForService('postgres-message', ['pg_isready', '-U', 'app', '-d', 'message'])
  await retryUntilPersisted(databaseMessage)
})
